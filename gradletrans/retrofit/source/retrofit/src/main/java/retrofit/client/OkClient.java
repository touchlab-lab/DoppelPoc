...
public class OkClient implements Client {
  ...
  static com.squareup.okhttp.Request createRequest(Request request) {
    ...
    List<ClientHeader> headers = request.getHeaders();
    for (int i = 0, size = headers.size(); i < size; i++) {
      ClientHeader header = headers.get(i);
  ...
  private static List<ClientHeader> createHeaders(Headers headers) {
    int size = headers.size();
    List<ClientHeader> headerList = new ArrayList<ClientHeader>(size);
    for (int i = 0; i < size; i++) {
      headerList.add(new ClientHeader(headers.name(i), headers.value(i)));
    }
    return headerList;
  }
}
