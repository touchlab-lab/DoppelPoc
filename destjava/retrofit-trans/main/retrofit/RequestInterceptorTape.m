//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: /Users/kgalligan/devel/xp/sourcejava/retrofit/retrofit/src/main/java/retrofit/RequestInterceptorTape.java
//

#include "IOSClass.h"
#include "J2ObjC_source.h"
#include "java/lang/Enum.h"
#include "java/lang/IllegalArgumentException.h"
#include "java/util/ArrayList.h"
#include "java/util/List.h"
#include "retrofit/RequestInterceptor.h"
#include "retrofit/RequestInterceptorTape.h"

@interface RetrofitRequestInterceptorTape () {
 @public
  id<JavaUtilList> tape_;
}

@end

J2OBJC_FIELD_SETTER(RetrofitRequestInterceptorTape, tape_, id<JavaUtilList>)

typedef NS_ENUM(NSUInteger, RetrofitRequestInterceptorTape_Command_Enum) {
  RetrofitRequestInterceptorTape_Command_Enum_ADD_HEADER = 0,
  RetrofitRequestInterceptorTape_Command_Enum_ADD_PATH_PARAM = 1,
  RetrofitRequestInterceptorTape_Command_Enum_ADD_ENCODED_PATH_PARAM = 2,
  RetrofitRequestInterceptorTape_Command_Enum_ADD_QUERY_PARAM = 3,
  RetrofitRequestInterceptorTape_Command_Enum_ADD_ENCODED_QUERY_PARAM = 4,
};

@interface RetrofitRequestInterceptorTape_Command : JavaLangEnum < NSCopying >

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value;

+ (IOSObjectArray *)values;

+ (RetrofitRequestInterceptorTape_Command *)valueOfWithNSString:(NSString *)name;

- (id)copyWithZone:(NSZone *)zone;

@end

J2OBJC_STATIC_INIT(RetrofitRequestInterceptorTape_Command)

/*! INTERNAL ONLY - Use enum accessors declared below. */
FOUNDATION_EXPORT RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_values_[];

inline RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_get_ADD_HEADER();
J2OBJC_ENUM_CONSTANT(RetrofitRequestInterceptorTape_Command, ADD_HEADER)

inline RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_get_ADD_PATH_PARAM();
J2OBJC_ENUM_CONSTANT(RetrofitRequestInterceptorTape_Command, ADD_PATH_PARAM)

inline RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_get_ADD_ENCODED_PATH_PARAM();
J2OBJC_ENUM_CONSTANT(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_PATH_PARAM)

inline RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_get_ADD_QUERY_PARAM();
J2OBJC_ENUM_CONSTANT(RetrofitRequestInterceptorTape_Command, ADD_QUERY_PARAM)

inline RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_get_ADD_ENCODED_QUERY_PARAM();
J2OBJC_ENUM_CONSTANT(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_QUERY_PARAM)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command *self, NSString *__name, jint __ordinal);

__attribute__((unused)) static RetrofitRequestInterceptorTape_Command *new_RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(NSString *__name, jint __ordinal) NS_RETURNS_RETAINED;

FOUNDATION_EXPORT IOSObjectArray *RetrofitRequestInterceptorTape_Command_values();

FOUNDATION_EXPORT RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_valueOfWithNSString_(NSString *name);

FOUNDATION_EXPORT RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_fromOrdinal(NSUInteger ordinal);

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_Command)

@interface RetrofitRequestInterceptorTape_Command_$1 : RetrofitRequestInterceptorTape_Command < NSCopying >

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value;

@end

J2OBJC_EMPTY_STATIC_INIT(RetrofitRequestInterceptorTape_Command_$1)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_Command_$1_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$1 *self, NSString *__name, jint __ordinal);

__attribute__((unused)) static RetrofitRequestInterceptorTape_Command_$1 *new_RetrofitRequestInterceptorTape_Command_$1_initWithNSString_withInt_(NSString *__name, jint __ordinal) NS_RETURNS_RETAINED;

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_Command_$1)

@interface RetrofitRequestInterceptorTape_Command_$2 : RetrofitRequestInterceptorTape_Command < NSCopying >

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value;

@end

J2OBJC_EMPTY_STATIC_INIT(RetrofitRequestInterceptorTape_Command_$2)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_Command_$2_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$2 *self, NSString *__name, jint __ordinal);

__attribute__((unused)) static RetrofitRequestInterceptorTape_Command_$2 *new_RetrofitRequestInterceptorTape_Command_$2_initWithNSString_withInt_(NSString *__name, jint __ordinal) NS_RETURNS_RETAINED;

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_Command_$2)

@interface RetrofitRequestInterceptorTape_Command_$3 : RetrofitRequestInterceptorTape_Command < NSCopying >

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value;

@end

J2OBJC_EMPTY_STATIC_INIT(RetrofitRequestInterceptorTape_Command_$3)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_Command_$3_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$3 *self, NSString *__name, jint __ordinal);

__attribute__((unused)) static RetrofitRequestInterceptorTape_Command_$3 *new_RetrofitRequestInterceptorTape_Command_$3_initWithNSString_withInt_(NSString *__name, jint __ordinal) NS_RETURNS_RETAINED;

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_Command_$3)

@interface RetrofitRequestInterceptorTape_Command_$4 : RetrofitRequestInterceptorTape_Command < NSCopying >

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value;

@end

J2OBJC_EMPTY_STATIC_INIT(RetrofitRequestInterceptorTape_Command_$4)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_Command_$4_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$4 *self, NSString *__name, jint __ordinal);

__attribute__((unused)) static RetrofitRequestInterceptorTape_Command_$4 *new_RetrofitRequestInterceptorTape_Command_$4_initWithNSString_withInt_(NSString *__name, jint __ordinal) NS_RETURNS_RETAINED;

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_Command_$4)

@interface RetrofitRequestInterceptorTape_Command_$5 : RetrofitRequestInterceptorTape_Command < NSCopying >

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value;

@end

J2OBJC_EMPTY_STATIC_INIT(RetrofitRequestInterceptorTape_Command_$5)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_Command_$5_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$5 *self, NSString *__name, jint __ordinal);

__attribute__((unused)) static RetrofitRequestInterceptorTape_Command_$5 *new_RetrofitRequestInterceptorTape_Command_$5_initWithNSString_withInt_(NSString *__name, jint __ordinal) NS_RETURNS_RETAINED;

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_Command_$5)

@interface RetrofitRequestInterceptorTape_CommandWithParams : NSObject {
 @public
  RetrofitRequestInterceptorTape_Command *command_;
  NSString *name_;
  NSString *value_;
}

- (instancetype)initWithRetrofitRequestInterceptorTape_Command:(RetrofitRequestInterceptorTape_Command *)command
                                                  withNSString:(NSString *)name
                                                  withNSString:(NSString *)value;

@end

J2OBJC_EMPTY_STATIC_INIT(RetrofitRequestInterceptorTape_CommandWithParams)

J2OBJC_FIELD_SETTER(RetrofitRequestInterceptorTape_CommandWithParams, command_, RetrofitRequestInterceptorTape_Command *)
J2OBJC_FIELD_SETTER(RetrofitRequestInterceptorTape_CommandWithParams, name_, NSString *)
J2OBJC_FIELD_SETTER(RetrofitRequestInterceptorTape_CommandWithParams, value_, NSString *)

__attribute__((unused)) static void RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(RetrofitRequestInterceptorTape_CommandWithParams *self, RetrofitRequestInterceptorTape_Command *command, NSString *name, NSString *value);

__attribute__((unused)) static RetrofitRequestInterceptorTape_CommandWithParams *new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(RetrofitRequestInterceptorTape_Command *command, NSString *name, NSString *value) NS_RETURNS_RETAINED;

__attribute__((unused)) static RetrofitRequestInterceptorTape_CommandWithParams *create_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(RetrofitRequestInterceptorTape_Command *command, NSString *name, NSString *value);

J2OBJC_TYPE_LITERAL_HEADER(RetrofitRequestInterceptorTape_CommandWithParams)

@implementation RetrofitRequestInterceptorTape

- (void)addHeaderWithNSString:(NSString *)name
                 withNSString:(NSString *)value {
  [((id<JavaUtilList>) nil_chk(tape_)) addWithId:new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(JreLoadEnum(RetrofitRequestInterceptorTape_Command, ADD_HEADER), name, value)];
}

- (void)addPathParamWithNSString:(NSString *)name
                    withNSString:(NSString *)value {
  [((id<JavaUtilList>) nil_chk(tape_)) addWithId:new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(JreLoadEnum(RetrofitRequestInterceptorTape_Command, ADD_PATH_PARAM), name, value)];
}

- (void)addEncodedPathParamWithNSString:(NSString *)name
                           withNSString:(NSString *)value {
  [((id<JavaUtilList>) nil_chk(tape_)) addWithId:new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(JreLoadEnum(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_PATH_PARAM), name, value)];
}

- (void)addQueryParamWithNSString:(NSString *)name
                     withNSString:(NSString *)value {
  [((id<JavaUtilList>) nil_chk(tape_)) addWithId:new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(JreLoadEnum(RetrofitRequestInterceptorTape_Command, ADD_QUERY_PARAM), name, value)];
}

- (void)addEncodedQueryParamWithNSString:(NSString *)name
                            withNSString:(NSString *)value {
  [((id<JavaUtilList>) nil_chk(tape_)) addWithId:new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(JreLoadEnum(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_QUERY_PARAM), name, value)];
}

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)request {
  for (RetrofitRequestInterceptorTape_CommandWithParams * __strong cwp in nil_chk(tape_)) {
    [((RetrofitRequestInterceptorTape_Command *) nil_chk(((RetrofitRequestInterceptorTape_CommandWithParams *) nil_chk(cwp))->command_)) interceptWithRetrofitRequestInterceptor_RequestFacade:request withNSString:cwp->name_ withNSString:cwp->value_];
  }
}

J2OBJC_IGNORE_DESIGNATED_BEGIN
- (instancetype)init {
  RetrofitRequestInterceptorTape_init(self);
  return self;
}
J2OBJC_IGNORE_DESIGNATED_END

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "addHeaderWithNSString:withNSString:", "addHeader", "V", 0x1, NULL, NULL },
    { "addPathParamWithNSString:withNSString:", "addPathParam", "V", 0x1, NULL, NULL },
    { "addEncodedPathParamWithNSString:withNSString:", "addEncodedPathParam", "V", 0x1, NULL, NULL },
    { "addQueryParamWithNSString:withNSString:", "addQueryParam", "V", 0x1, NULL, NULL },
    { "addEncodedQueryParamWithNSString:withNSString:", "addEncodedQueryParam", "V", 0x1, NULL, NULL },
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:", "intercept", "V", 0x1, NULL, NULL },
    { "init", NULL, NULL, 0x0, NULL, NULL },
  };
  static const J2ObjcFieldInfo fields[] = {
    { "tape_", NULL, 0x12, "Ljava.util.List;", NULL, "Ljava/util/List<Lretrofit/RequestInterceptorTape$CommandWithParams;>;", .constantValue.asLong = 0 },
  };
  static const char *inner_classes[] = {"Lretrofit.RequestInterceptorTape$Command;", "Lretrofit.RequestInterceptorTape$CommandWithParams;"};
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape = { 2, "RequestInterceptorTape", "retrofit", NULL, 0x10, 7, methods, 1, fields, 0, NULL, 2, inner_classes, NULL, NULL };
  return &_RetrofitRequestInterceptorTape;
}

@end

void RetrofitRequestInterceptorTape_init(RetrofitRequestInterceptorTape *self) {
  NSObject_init(self);
  self->tape_ = new_JavaUtilArrayList_init();
}

RetrofitRequestInterceptorTape *new_RetrofitRequestInterceptorTape_init() {
  RetrofitRequestInterceptorTape *self = [RetrofitRequestInterceptorTape alloc];
  RetrofitRequestInterceptorTape_init(self);
  return self;
}

RetrofitRequestInterceptorTape *create_RetrofitRequestInterceptorTape_init() {
  return new_RetrofitRequestInterceptorTape_init();
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape)

J2OBJC_INITIALIZED_DEFN(RetrofitRequestInterceptorTape_Command)

RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_values_[5];

@implementation RetrofitRequestInterceptorTape_Command

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value {
  // can't call an abstract method
  [self doesNotRecognizeSelector:_cmd];
}

+ (IOSObjectArray *)values {
  return RetrofitRequestInterceptorTape_Command_values();
}

+ (RetrofitRequestInterceptorTape_Command *)valueOfWithNSString:(NSString *)name {
  return RetrofitRequestInterceptorTape_Command_valueOfWithNSString_(name);
}

- (id)copyWithZone:(NSZone *)zone {
  return self;
}

+ (void)initialize {
  if (self == [RetrofitRequestInterceptorTape_Command class]) {
    JreEnum(RetrofitRequestInterceptorTape_Command, ADD_HEADER) = new_RetrofitRequestInterceptorTape_Command_$1_initWithNSString_withInt_(@"ADD_HEADER", 0);
    JreEnum(RetrofitRequestInterceptorTape_Command, ADD_PATH_PARAM) = new_RetrofitRequestInterceptorTape_Command_$2_initWithNSString_withInt_(@"ADD_PATH_PARAM", 1);
    JreEnum(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_PATH_PARAM) = new_RetrofitRequestInterceptorTape_Command_$3_initWithNSString_withInt_(@"ADD_ENCODED_PATH_PARAM", 2);
    JreEnum(RetrofitRequestInterceptorTape_Command, ADD_QUERY_PARAM) = new_RetrofitRequestInterceptorTape_Command_$4_initWithNSString_withInt_(@"ADD_QUERY_PARAM", 3);
    JreEnum(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_QUERY_PARAM) = new_RetrofitRequestInterceptorTape_Command_$5_initWithNSString_withInt_(@"ADD_ENCODED_QUERY_PARAM", 4);
    J2OBJC_SET_INITIALIZED(RetrofitRequestInterceptorTape_Command)
  }
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:withNSString:withNSString:", "intercept", "V", 0x400, NULL, NULL },
  };
  static const J2ObjcFieldInfo fields[] = {
    { "ADD_HEADER", "ADD_HEADER", 0x4019, "Lretrofit.RequestInterceptorTape$Command;", &JreEnum(RetrofitRequestInterceptorTape_Command, ADD_HEADER), NULL, .constantValue.asLong = 0 },
    { "ADD_PATH_PARAM", "ADD_PATH_PARAM", 0x4019, "Lretrofit.RequestInterceptorTape$Command;", &JreEnum(RetrofitRequestInterceptorTape_Command, ADD_PATH_PARAM), NULL, .constantValue.asLong = 0 },
    { "ADD_ENCODED_PATH_PARAM", "ADD_ENCODED_PATH_PARAM", 0x4019, "Lretrofit.RequestInterceptorTape$Command;", &JreEnum(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_PATH_PARAM), NULL, .constantValue.asLong = 0 },
    { "ADD_QUERY_PARAM", "ADD_QUERY_PARAM", 0x4019, "Lretrofit.RequestInterceptorTape$Command;", &JreEnum(RetrofitRequestInterceptorTape_Command, ADD_QUERY_PARAM), NULL, .constantValue.asLong = 0 },
    { "ADD_ENCODED_QUERY_PARAM", "ADD_ENCODED_QUERY_PARAM", 0x4019, "Lretrofit.RequestInterceptorTape$Command;", &JreEnum(RetrofitRequestInterceptorTape_Command, ADD_ENCODED_QUERY_PARAM), NULL, .constantValue.asLong = 0 },
  };
  static const char *superclass_type_args[] = {"Lretrofit.RequestInterceptorTape$Command;"};
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_Command = { 2, "Command", "retrofit", "RequestInterceptorTape", 0x440a, 1, methods, 5, fields, 1, superclass_type_args, 0, NULL, NULL, "Ljava/lang/Enum<Lretrofit/RequestInterceptorTape$Command;>;" };
  return &_RetrofitRequestInterceptorTape_Command;
}

@end

void RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command *self, NSString *__name, jint __ordinal) {
  JavaLangEnum_initWithNSString_withInt_(self, __name, __ordinal);
}

RetrofitRequestInterceptorTape_Command *new_RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command *self = [RetrofitRequestInterceptorTape_Command alloc];
  RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(self, __name, __ordinal);
  return self;
}

IOSObjectArray *RetrofitRequestInterceptorTape_Command_values() {
  RetrofitRequestInterceptorTape_Command_initialize();
  return [IOSObjectArray arrayWithObjects:RetrofitRequestInterceptorTape_Command_values_ count:5 type:RetrofitRequestInterceptorTape_Command_class_()];
}

RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_valueOfWithNSString_(NSString *name) {
  RetrofitRequestInterceptorTape_Command_initialize();
  for (int i = 0; i < 5; i++) {
    RetrofitRequestInterceptorTape_Command *e = RetrofitRequestInterceptorTape_Command_values_[i];
    if ([name isEqual:[e name]]) {
      return e;
    }
  }
  @throw [[JavaLangIllegalArgumentException alloc] initWithNSString:name];
  return nil;
}

RetrofitRequestInterceptorTape_Command *RetrofitRequestInterceptorTape_Command_fromOrdinal(NSUInteger ordinal) {
  RetrofitRequestInterceptorTape_Command_initialize();
  if (ordinal >= 5) {
    return nil;
  }
  return RetrofitRequestInterceptorTape_Command_values_[ordinal];
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_Command)

@implementation RetrofitRequestInterceptorTape_Command_$1

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value {
  [((id<RetrofitRequestInterceptor_RequestFacade>) nil_chk(facade)) addHeaderWithNSString:name withNSString:value];
}

- (void)dealloc {
  JreCheckFinalize(self, [RetrofitRequestInterceptorTape_Command_$1 class]);
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:withNSString:withNSString:", "intercept", "V", 0x1, NULL, NULL },
  };
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_Command_$1 = { 2, "", "retrofit", "RequestInterceptorTape$Command", 0xc018, 1, methods, 0, NULL, 0, NULL, 0, NULL, NULL, NULL };
  return &_RetrofitRequestInterceptorTape_Command_$1;
}

@end

void RetrofitRequestInterceptorTape_Command_$1_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$1 *self, NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(self, __name, __ordinal);
}

RetrofitRequestInterceptorTape_Command_$1 *new_RetrofitRequestInterceptorTape_Command_$1_initWithNSString_withInt_(NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_$1 *self = [RetrofitRequestInterceptorTape_Command_$1 alloc];
  RetrofitRequestInterceptorTape_Command_$1_initWithNSString_withInt_(self, __name, __ordinal);
  return self;
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_Command_$1)

@implementation RetrofitRequestInterceptorTape_Command_$2

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value {
  [((id<RetrofitRequestInterceptor_RequestFacade>) nil_chk(facade)) addPathParamWithNSString:name withNSString:value];
}

- (void)dealloc {
  JreCheckFinalize(self, [RetrofitRequestInterceptorTape_Command_$2 class]);
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:withNSString:withNSString:", "intercept", "V", 0x1, NULL, NULL },
  };
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_Command_$2 = { 2, "", "retrofit", "RequestInterceptorTape$Command", 0xc018, 1, methods, 0, NULL, 0, NULL, 0, NULL, NULL, NULL };
  return &_RetrofitRequestInterceptorTape_Command_$2;
}

@end

void RetrofitRequestInterceptorTape_Command_$2_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$2 *self, NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(self, __name, __ordinal);
}

RetrofitRequestInterceptorTape_Command_$2 *new_RetrofitRequestInterceptorTape_Command_$2_initWithNSString_withInt_(NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_$2 *self = [RetrofitRequestInterceptorTape_Command_$2 alloc];
  RetrofitRequestInterceptorTape_Command_$2_initWithNSString_withInt_(self, __name, __ordinal);
  return self;
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_Command_$2)

@implementation RetrofitRequestInterceptorTape_Command_$3

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value {
  [((id<RetrofitRequestInterceptor_RequestFacade>) nil_chk(facade)) addEncodedPathParamWithNSString:name withNSString:value];
}

- (void)dealloc {
  JreCheckFinalize(self, [RetrofitRequestInterceptorTape_Command_$3 class]);
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:withNSString:withNSString:", "intercept", "V", 0x1, NULL, NULL },
  };
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_Command_$3 = { 2, "", "retrofit", "RequestInterceptorTape$Command", 0xc018, 1, methods, 0, NULL, 0, NULL, 0, NULL, NULL, NULL };
  return &_RetrofitRequestInterceptorTape_Command_$3;
}

@end

void RetrofitRequestInterceptorTape_Command_$3_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$3 *self, NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(self, __name, __ordinal);
}

RetrofitRequestInterceptorTape_Command_$3 *new_RetrofitRequestInterceptorTape_Command_$3_initWithNSString_withInt_(NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_$3 *self = [RetrofitRequestInterceptorTape_Command_$3 alloc];
  RetrofitRequestInterceptorTape_Command_$3_initWithNSString_withInt_(self, __name, __ordinal);
  return self;
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_Command_$3)

@implementation RetrofitRequestInterceptorTape_Command_$4

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value {
  [((id<RetrofitRequestInterceptor_RequestFacade>) nil_chk(facade)) addQueryParamWithNSString:name withNSString:value];
}

- (void)dealloc {
  JreCheckFinalize(self, [RetrofitRequestInterceptorTape_Command_$4 class]);
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:withNSString:withNSString:", "intercept", "V", 0x1, NULL, NULL },
  };
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_Command_$4 = { 2, "", "retrofit", "RequestInterceptorTape$Command", 0xc018, 1, methods, 0, NULL, 0, NULL, 0, NULL, NULL, NULL };
  return &_RetrofitRequestInterceptorTape_Command_$4;
}

@end

void RetrofitRequestInterceptorTape_Command_$4_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$4 *self, NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(self, __name, __ordinal);
}

RetrofitRequestInterceptorTape_Command_$4 *new_RetrofitRequestInterceptorTape_Command_$4_initWithNSString_withInt_(NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_$4 *self = [RetrofitRequestInterceptorTape_Command_$4 alloc];
  RetrofitRequestInterceptorTape_Command_$4_initWithNSString_withInt_(self, __name, __ordinal);
  return self;
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_Command_$4)

@implementation RetrofitRequestInterceptorTape_Command_$5

- (void)interceptWithRetrofitRequestInterceptor_RequestFacade:(id<RetrofitRequestInterceptor_RequestFacade>)facade
                                                 withNSString:(NSString *)name
                                                 withNSString:(NSString *)value {
  [((id<RetrofitRequestInterceptor_RequestFacade>) nil_chk(facade)) addEncodedQueryParamWithNSString:name withNSString:value];
}

- (void)dealloc {
  JreCheckFinalize(self, [RetrofitRequestInterceptorTape_Command_$5 class]);
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "interceptWithRetrofitRequestInterceptor_RequestFacade:withNSString:withNSString:", "intercept", "V", 0x1, NULL, NULL },
  };
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_Command_$5 = { 2, "", "retrofit", "RequestInterceptorTape$Command", 0xc018, 1, methods, 0, NULL, 0, NULL, 0, NULL, NULL, NULL };
  return &_RetrofitRequestInterceptorTape_Command_$5;
}

@end

void RetrofitRequestInterceptorTape_Command_$5_initWithNSString_withInt_(RetrofitRequestInterceptorTape_Command_$5 *self, NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_initWithNSString_withInt_(self, __name, __ordinal);
}

RetrofitRequestInterceptorTape_Command_$5 *new_RetrofitRequestInterceptorTape_Command_$5_initWithNSString_withInt_(NSString *__name, jint __ordinal) {
  RetrofitRequestInterceptorTape_Command_$5 *self = [RetrofitRequestInterceptorTape_Command_$5 alloc];
  RetrofitRequestInterceptorTape_Command_$5_initWithNSString_withInt_(self, __name, __ordinal);
  return self;
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_Command_$5)

@implementation RetrofitRequestInterceptorTape_CommandWithParams

- (instancetype)initWithRetrofitRequestInterceptorTape_Command:(RetrofitRequestInterceptorTape_Command *)command
                                                  withNSString:(NSString *)name
                                                  withNSString:(NSString *)value {
  RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(self, command, name, value);
  return self;
}

+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    { "initWithRetrofitRequestInterceptorTape_Command:withNSString:withNSString:", "CommandWithParams", NULL, 0x0, NULL, NULL },
  };
  static const J2ObjcFieldInfo fields[] = {
    { "command_", NULL, 0x10, "Lretrofit.RequestInterceptorTape$Command;", NULL, NULL, .constantValue.asLong = 0 },
    { "name_", NULL, 0x10, "Ljava.lang.String;", NULL, NULL, .constantValue.asLong = 0 },
    { "value_", NULL, 0x10, "Ljava.lang.String;", NULL, NULL, .constantValue.asLong = 0 },
  };
  static const J2ObjcClassInfo _RetrofitRequestInterceptorTape_CommandWithParams = { 2, "CommandWithParams", "retrofit", "RequestInterceptorTape", 0x1a, 1, methods, 3, fields, 0, NULL, 0, NULL, NULL, NULL };
  return &_RetrofitRequestInterceptorTape_CommandWithParams;
}

@end

void RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(RetrofitRequestInterceptorTape_CommandWithParams *self, RetrofitRequestInterceptorTape_Command *command, NSString *name, NSString *value) {
  NSObject_init(self);
  self->command_ = command;
  self->name_ = name;
  self->value_ = value;
}

RetrofitRequestInterceptorTape_CommandWithParams *new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(RetrofitRequestInterceptorTape_Command *command, NSString *name, NSString *value) {
  RetrofitRequestInterceptorTape_CommandWithParams *self = [RetrofitRequestInterceptorTape_CommandWithParams alloc];
  RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(self, command, name, value);
  return self;
}

RetrofitRequestInterceptorTape_CommandWithParams *create_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(RetrofitRequestInterceptorTape_Command *command, NSString *name, NSString *value) {
  return new_RetrofitRequestInterceptorTape_CommandWithParams_initWithRetrofitRequestInterceptorTape_Command_withNSString_withNSString_(command, name, value);
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(RetrofitRequestInterceptorTape_CommandWithParams)
