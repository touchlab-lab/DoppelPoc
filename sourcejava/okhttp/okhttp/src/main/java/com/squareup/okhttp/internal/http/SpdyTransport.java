...
import static com.squareup.okhttp.internal.spdy.SpdyHeader.RESPONSE_STATUS;
import static com.squareup.okhttp.internal.spdy.SpdyHeader.TARGET_AUTHORITY;
import static com.squareup.okhttp.internal.spdy.SpdyHeader.TARGET_HOST;
import static com.squareup.okhttp.internal.spdy.SpdyHeader.TARGET_METHOD;
import static com.squareup.okhttp.internal.spdy.SpdyHeader.TARGET_PATH;
import static com.squareup.okhttp.internal.spdy.SpdyHeader.TARGET_SCHEME;
import static com.squareup.okhttp.internal.spdy.SpdyHeader.VERSION;
...
public final class SpdyTransport implements Transport {
   ...
   */
  public static List<SpdyHeader> writeNameValueBlock(Request request, Protocol protocol,
                                                     String version) {
    Headers headers = request.headers();
    List<SpdyHeader> result = new ArrayList<>(headers.size() + 10);
    result.add(new SpdyHeader(TARGET_METHOD, request.method()));
    result.add(new SpdyHeader(TARGET_PATH, RequestLine.requestPath(request.url())));
    String host = HttpEngine.hostHeader(request.url());
    if (Protocol.SPDY_3 == protocol) {
      result.add(new SpdyHeader(VERSION, version));
      result.add(new SpdyHeader(TARGET_HOST, host));
    } else if (Protocol.HTTP_2 == protocol) {
      result.add(new SpdyHeader(TARGET_AUTHORITY, host)); // Optional in HTTP/2
    } else {
      throw new AssertionError();
    }
    result.add(new SpdyHeader(TARGET_SCHEME, request.url().getProtocol()));
    ...
    for (int i = 0, size = headers.size(); i < size; i++) {
      ...
      if (names.add(name)) {
        result.add(new SpdyHeader(name, value));
        continue;
      }
      ...
      for (int j = 0; j < result.size(); j++) {
        if (result.get(j).name.equals(name)) {
          ...
          result.set(j, new SpdyHeader(name, concatenated));
          break;
        }
      }
    }
  ...
  public static Response.Builder readNameValueBlock(List<SpdyHeader> headerBlock,
