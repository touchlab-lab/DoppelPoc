...
public final class Http20Draft16 implements Variant {
  ...
  static final class Reader implements FrameReader {
    ...
    private void readHeaders(Handler handler, int length, byte flags, int streamId)
      ...
      List<SpdyHeader> headerBlock = readHeaderBlock(length, padding, flags, streamId);

      handler.headers(false, endStream, streamId, -1, headerBlock, HeadersMode.HTTP_20_HEADERS);
    }

    private List<SpdyHeader> readHeaderBlock(int length, short padding, byte flags, int streamId)
        throws IOException {
  ...
  static final class Writer implements FrameWriter {
    ...
    @Override public synchronized void pushPromise(int streamId, int promisedStreamId,
        List<SpdyHeader> requestHeaders) throws IOException {
      if (closed) throw new IOException("closed");
    ...
    void headers(boolean outFinished, int streamId, List<SpdyHeader> headerBlock) throws IOException {
      if (closed) throw new IOException("closed");
      if (hpackBuffer.size() != 0) throw new IllegalStateException();
