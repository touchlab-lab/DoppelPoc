...
public final class Http2ConnectionTest {
  ...
  @Test public void pushPromiseStream() throws Exception {
    ...
    peer.acceptFrame(); // SYN_STREAM
    peer.sendFrame().synReply(false, 3, headerEntries("a", "android"));
    final List<SpdyHeader> expectedRequestHeaders = Arrays.asList(
        ...
        new SpdyHeader(SpdyHeader.TARGET_PATH, "/cached")
    );
    peer.sendFrame().pushPromise(3, 2, expectedRequestHeaders);
    final List<SpdyHeader> expectedResponseHeaders = Arrays.asList(
  ...
  static final PushObserver IGNORE = new PushObserver() {
    ...
    @Override public boolean onRequest(int streamId, List<SpdyHeader> requestHeaders) {
      return false;
    }

    @Override public boolean onHeaders(int streamId, List<SpdyHeader> responseHeaders, boolean last) {
      return false;
    }
  ...
  private static class RecordingPushObserver implements PushObserver {
    ...
    @Override public synchronized boolean onRequest(int streamId, List<SpdyHeader> requestHeaders) {
    ...
    @Override public synchronized boolean onHeaders(
            int streamId, List<SpdyHeader> responseHeaders, boolean last) {
      assertEquals(2, streamId);
      assertTrue(last);
