...
public class Http20Draft16Test {
  ...
  @Test public void onlyOneLiteralHeadersFrame() throws IOException {
    final List<SpdyHeader> sentHeaders = headerEntries("name", "value");
    ...
    fr.nextFrame(new BaseTestHandler() {
      ...
      public void headers(boolean outFinished, boolean inFinished, int streamId,
                          int associatedStreamId, List<SpdyHeader> headerBlock, HeadersMode headersMode) {
  ...
  @Test public void headersWithPriority() throws IOException {
    ...
    fr.nextFrame(new BaseTestHandler() {
      ...
      @Override public void headers(boolean outFinished, boolean inFinished, int streamId,
          int associatedStreamId, List<SpdyHeader> nameValueBlock,
  ...
  @Test public void headersFrameThenContinuation() throws IOException {
    final List<SpdyHeader> sentHeaders = largeHeaders();
  ...
  @Test public void pushPromise() throws IOException {
    ...
    final List<SpdyHeader> pushPromise = Arrays.asList(
        new SpdyHeader(SpdyHeader.TARGET_METHOD, "GET"),
        new SpdyHeader(SpdyHeader.TARGET_SCHEME, "https"),
        new SpdyHeader(SpdyHeader.TARGET_AUTHORITY, "squareup.com"),
    ...
    fr.nextFrame(new BaseTestHandler() {
      @Override
      public void pushPromise(int streamId, int promisedStreamId, List<SpdyHeader> headerBlock) {
        assertEquals(expectedStreamId, streamId);
  ...
  @Test public void pushPromiseThenContinuation() throws IOException {
    final int expectedPromisedStreamId = 11;
    final List<SpdyHeader> pushPromise = largeHeaders();
  ...
  private Buffer literalHeaders(List<SpdyHeader> sentHeaders) throws IOException {
    Buffer out = new Buffer();
    new HpackDraft10.Writer(out).writeHeaders(sentHeaders);
    return out;
  }

  private Buffer sendHeaderFrames(boolean outFinished, List<SpdyHeader> headers) throws IOException {
    Buffer out = new Buffer();
    new Http20Draft16.Writer(out, true).headers(outFinished, expectedStreamId, headers);
    return out;
  }

  private Buffer sendPushPromiseFrames(int streamId, List<SpdyHeader> headers) throws IOException {
    Buffer out = new Buffer();
  ...
  private static List<SpdyHeader> largeHeaders() {
