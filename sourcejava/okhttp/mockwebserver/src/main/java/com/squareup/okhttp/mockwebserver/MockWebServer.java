  ...
  *        {@code unit}
  ...
  private class SpdySocketHandler implements IncomingStreamHandler {
    ...
    private RecordedRequest readRequest(SpdyStream stream) throws IOException {
      List<SpdyHeader> spdyHeaders = stream.getRequestHeaders();
      ...
      for (int i = 0, size = spdyHeaders.size(); i < size; i++) {
        ...
        if (name.equals(SpdyHeader.TARGET_METHOD)) {
          method = value;
        } else if (name.equals(SpdyHeader.TARGET_PATH)) {
          path = value;
        } else if (name.equals(SpdyHeader.VERSION)) {
          version = value;
        } else {
    ...
    private void writeResponse(SpdyStream stream, MockResponse response) throws IOException {
      ...
      List<SpdyHeader> spdyHeaders = new ArrayList<>();
      String[] statusParts = response.getStatus().split(" ", 2);
      if (statusParts.length != 2) {
        throw new AssertionError("Unexpected status: " + response.getStatus());
      }
      // TODO: constants for well-known header names.
      spdyHeaders.add(new SpdyHeader(SpdyHeader.RESPONSE_STATUS, statusParts[1]));
      if (protocol == Protocol.SPDY_3) {
        spdyHeaders.add(new SpdyHeader(SpdyHeader.VERSION, statusParts[0]));
      }
      List<String> headers = response.getHeaders();
      for (int i = 0, size = headers.size(); i < size; i++) {
        String header = headers.get(i);
        ...
        spdyHeaders.add(new SpdyHeader(headerParts[0], headerParts[1]));
      }
    ...
    private void pushPromises(SpdyStream stream, List<PushPromise> promises) throws IOException {
      for (PushPromise pushPromise : promises) {
        List<SpdyHeader> pushedHeaders = new ArrayList<>();
        pushedHeaders.add(new SpdyHeader(stream.getConnection().getProtocol() == Protocol.SPDY_3
            ? SpdyHeader.TARGET_HOST
            : SpdyHeader.TARGET_AUTHORITY, getUrl(pushPromise.getPath()).getHost()));
        pushedHeaders.add(new SpdyHeader(SpdyHeader.TARGET_METHOD, pushPromise.getMethod()));
        pushedHeaders.add(new SpdyHeader(SpdyHeader.TARGET_PATH, pushPromise.getPath()));
        for (int i = 0, size = pushPromise.getHeaders().size(); i < size; i++) {
          ...
          pushedHeaders.add(new SpdyHeader(headerParts[0], headerParts[1].trim()));
        }
