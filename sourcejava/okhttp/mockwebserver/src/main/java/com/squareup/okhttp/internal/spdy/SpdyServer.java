...
public final class SpdyServer implements IncomingStreamHandler {
  ...
  @Override public void receive(final SpdyStream stream) throws IOException {
    List<SpdyHeader> requestHeaders = stream.getRequestHeaders();
    String path = null;
    for (int i = 0, size = requestHeaders.size(); i < size; i++) {
      if (requestHeaders.get(i).name.equals(SpdyHeader.TARGET_PATH)) {
  ...
  private void send404(SpdyStream stream, String path) throws IOException {
    List<SpdyHeader> responseHeaders = Arrays.asList(
        new SpdyHeader(":status", "404"),
        new SpdyHeader(":version", "HTTP/1.1"),
        new SpdyHeader("content-type", "text/plain")
    );
  ...
  private void serveDirectory(SpdyStream stream, String[] files) throws IOException {
    List<SpdyHeader> responseHeaders = Arrays.asList(
        new SpdyHeader(":status", "200"),
        new SpdyHeader(":version", "HTTP/1.1"),
        new SpdyHeader("content-type", "text/html; charset=UTF-8")
    );
    stream.reply(responseHeaders, true);
    BufferedSink out = Okio.buffer(stream.getSink());
  ...
  private void serveFile(SpdyStream stream, File file) throws IOException {
    List<SpdyHeader> responseHeaders = Arrays.asList(
        new SpdyHeader(":status", "200"),
        new SpdyHeader(":version", "HTTP/1.1"),
        new SpdyHeader("content-type", contentType(file))
    );
    stream.reply(responseHeaders, true);
